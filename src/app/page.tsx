      ...
      <header className="shrink-0 px-6 py-4 flex items-center gap-4 border-b border-gray-200/50 bg-white/60 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400">키워드 입력 → 상위노출 분석 → 원고 자동 생성</p>
          </div>
        </div>
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                className="flex-1 py-3 bg-gradient-to-r from-mint-500 to-mint-600 text-white font-semibold rounded-xl hover:from-mint-600 hover:to-mint-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all btn-glow-mint flex items-center justify-center gap-2 text-sm"
              >
              ...
              <button
                ...
                className="flex-1 py-3 bg-gradient-to-r from-primary-500 to-primary-600 text-white font-semibold rounded-xl hover:from-primary-600 hover:to-primary-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all btn-glow-blue flex items-center justify-center gap-2 text-sm"
              >
                {isGenerating ? <><Spinner /> 생성 중...</> : (
                  <>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
            ...
            {crawlDone && (
              <div className="pt-3 border-t border-gray-100">
                <div className="flex items-center gap-2 mb-3">
                  <h3 className="text-xs font-bold text-gray-600">상위노출 분석</h3>
                  <span className="text-[10px] font-medium text-mint-600 bg-mint-50 px-2 py-0.5 rounded-full">
                    {crawlResults.length}건
                  </span>
                </div>
                {crawlResults.length === 0 ? (
                  <p className="text-gray-400 text-xs text-center py-4">결과가 없어요.</p>
                ) : (
                  <div className="space-y-1.5">
                    {crawlResults.map((item, idx) => (
                      <details key={idx} className="group rounded-xl border border-gray-100 overflow-hidden hover:border-mint-200 transition-colors">
                        <summary className="px-3 py-2.5 cursor-pointer hover:bg-gray-50/50 text-xs text-gray-600 flex items-center gap-2">
                          ...
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 text-gray-300 ml-auto shrink-0 group-open:rotate-180 transition-transform" viewBox="0 0 20 20" fill="currentColor">
          ...
          {generatedContent ? (
            <div className="h-full flex flex-col">
              ...
              <div className="shrink-0 px-6 py-3.5 border-b border-gray-200/50 bg-white/40 backdrop-blur-sm flex items-center">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-gray-400 ml-1">{generatedContent.length}자</span>
                </div>
                <button
                  onClick={handleCopy}
            ...
            <div className="h-full flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-xs text-gray-300">왼쪽에서 키워드를 입력하고 원고를 생성해보세요</p>
              </div>
            </div>
          )}
        </main>
